...
const string strMessageMagic = "Abundance Signed Message:\n";
...
const int YEARLY_BLOCKCOUNT =  1051200;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 <= (2 * MONTHLY_BLOCKCOUNT))
    {
      nRewardCoinYear = 4 * MAX_MINT_PROOF_OF_STAKE;
    }
    
    else if (pindexBest->nHeight+1 <= (3 * MONTHLY_BLOCKCOUNT))
    {
      nRewardCoinYear = 3 * MAX_MINT_PROOF_OF_STAKE;
    }
    
    else if (pindexBest->nHeight+1 <= (4 * MONTHLY_BLOCKCOUNT))
    {
      nRewardCoinYear = 2 * MAX_MINT_PROOF_OF_STAKE;
    }
    
    else if (pindexBest->nHeight+1 <= (5 * MONTHLY_BLOCKCOUNT))
    {
      nRewardCoinYear = 1 * MAX_MINT_PROOF_OF_STAKE;
    }
    
    else if (pindexBest->nHeight+1 <= YEARLY_BLOCKCOUNT)
    {
      nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE2;
    }
    
    else if (pindexBest->nHeight+1 <= (4 * YEARLY_BLOCKCOUNT))
    {
      nRewardCoinYear = 2 * MAX_MINT_PROOF_OF_STAKE;
    }
...
static const int64_t nTargetTimespan = 10 * 60;  // 10 mins or 20 blocks
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Abundance", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Wed, 20 Aug 2014 20:16:30 GMT";
        CTransaction txNew;
        txNew.nTime = 1408565792;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1408565792;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 6857098;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xcd05b02768734a837b02c801ce864493ec49ac73b748fc268b91a669f320ae73"));
        block.print();
